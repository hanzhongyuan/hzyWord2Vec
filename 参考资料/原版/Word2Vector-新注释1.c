...
//词表，对于一个给定的单词word，需要通过vocab_hash得到这个词在vocab里的下标k，然后才能通过vocab[k]进行访问
//如果是其他语言，完全可以用dictionary代替,
//例如“你好”存在vocab【3】中，这个词的ascii码是123456, vocab_hash[123456]=3
//则“你好”先去 vocab_hash[123456]取到3，然后vocab[3]得到"你好"的结构体
...
int* table; //table 存储的是根据词频生成的负采样样本
...
void InitUnigramTable() {
	...
	//为词频分布表分配内存空间，共有table_size项，table_size为一个既定的数1e8
	table = (int*)malloc(table_size * sizeof(int));

	//遍历词表，根据词频计算累计值的分母
	for (a = 0; a < vocab_size; a++)
	{
		train_words_pow += pow(vocab[a].cn, power); //  求和( 词频^0.75)
	}
	...
	for (a = 0; a < table_size; a++) {		
		table[a] = i; //将一段从a+0到a+dl*1e8之间的设置成i		
